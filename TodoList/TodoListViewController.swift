...
//        print("---> restore from disk: \(todo)")
    ...
    @IBAction func addTaskButtonTapped(_ sender: Any) {
        ...
        let todo = TodoManager.shared.createTodo(detail: detail, isToday:
...
extension TodoListViewController {
    @objc private func adjustInputView(noti: Notification) {
        ...
        guard let keyboaredFrame = (userInfo[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue else {return}
        
        if noti.name == UIResponder.keyboardWillShowNotification {
            let adjustmentHeight = keyboaredFrame.height - view.safeAreaInsets.bottom
            inputViewBottom.constant = adjustmentHeight
        } else {
            inputViewBottom.constant = 0
        }
        
        print("---> Keyboard End Frame: \(keyboaredFrame)")
    }
}
